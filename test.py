...
def main():
    ...
    image = cv2.imread('../Dataset/VOC2012/IMAGES/2007_000862.jpg')
    ...
    pred_bbox = model.predict(image_data[np.newaxis, ...])
    pred_bbox = [tf.reshape(x, (-1, tf.shape(x)[-1])) for x in pred_bbox]
    pred_bbox = tf.concat(pred_bbox, axis=0)
    bbox = postprocess_boxes(pred_bbox, image_size, config.image_size, 0.25)
    bbox = nms(bbox)
